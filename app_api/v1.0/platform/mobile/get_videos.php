...
$featured = $db->where('approved', '1')->get($table,$limit,array('video_id','user_id'));

if (empty($featured)) {
	if (!empty($get_params['featured_offset'])) {
		$db->where('id', $get_params['featured_offset'],'<');
	}
    $featured = $db->where('approved', '1')->orderBy('id', 'DESC')->get(T_VIDEOS,$limit,array('video_id','user_id'));
}
...
$top   = $db->where('approved', '1')->orderby('views', 'DESC')->get(T_VIDEOS, $limit,array('video_id','user_id'));
...
$limit  = ((!empty($get_params['limit'])) ? $get_params['limit'] : 10);
$latest = $db->where('approved', '1')->orderby('id', 'DESC')->get(T_VIDEOS, $limit,array('video_id','user_id'));
