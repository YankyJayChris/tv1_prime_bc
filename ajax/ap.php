...
if ($first == 'videos-status') {
    if (!empty($_POST['id']) && !empty($_POST['status'])) {
        ...
        $update_data = $db->where('id', PT_Secure($_POST['id']))->update(T_VIDEOS, array('approved' => $update_to));
        if ($update_data) {
            $data["status"] = 200;
            $mydata = $db->where('id', PT_Secure($_POST['id']))->getOne(T_VIDEOS);
            if (!empty($mydata)) {
...
if ($first == 'delete_category' && !empty($_POST['lang_key'])) {
    if ($_POST['lang_key'] != 'other') {
        ...
        $db->where('category_id', PT_Secure($_POST['lang_key']))->update(T_VIDEOS, array(
            'sub_category' => '',
            'category_id' => 'other'
        ));
        $data['status'] = 200;
...
if ($first == 'insert-invitation') {
    ...
    $invitation_id = $db->insert(T_INVITATIONS, array(
        'code' => $code,
        'posted' => $time
    ));
